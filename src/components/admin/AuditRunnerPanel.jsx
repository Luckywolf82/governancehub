...
export default function AuditRunnerPanel({ onUseInOrchestrator }) {
  ...
  // possibly resolved / status unknown → Section B. All others → Section A.
  const verifiedNeedsFollowUp = useMemo(
    () => verifiedEntries.filter((e) => !e.githubIssueId && !e.githubStatus),
    [verifiedEntries]
  );
  const verifiedUnknownStatus = useMemo(
    () => verifiedEntries.filter((e) => e.githubIssueId || e.githubStatus),
    [verifiedEntries]
  );
        ...
        {showVerifiedAudits && (
          <CardContent className="pt-0 space-y-4">
            ...
            <div className="flex items-start gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded px-3 py-2">
              ...
              <span>Audit-status er ikke synkronisert med GitHub enda. Unngå duplikate oppgaver.</span>
            </div>
            ...
            {/* Section A — needs follow-up */}
            {verifiedNeedsFollowUp.length > 0 && (
              <div className="space-y-3">
                <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Verified audits – needs follow-up</p>
                {verifiedNeedsFollowUp.map((entry) => (
            ...
            {/* Section B — unknown / possibly resolved */}
            {verifiedUnknownStatus.length > 0 && (
              <div className="space-y-3">
                <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Verified audits – unknown / possibly resolved</p>
                {verifiedUnknownStatus.map((entry) => (
                  <div key={entry.id} className="border border-slate-100 rounded p-3 space-y-2">
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-xs font-mono bg-green-50 text-green-700 border border-green-200 rounded px-1.5 py-0.5">{entry.id}</span>
                    ...
                    <p className="text-xs text-slate-400 italic">Oppfølging ikke tilgjengelig (status ukjent)</p>
                  </div>
                ))}
              </div>
            )}
