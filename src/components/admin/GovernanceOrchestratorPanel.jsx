          ...
          {showLogAssistant && (
            <CardContent className="pt-0 space-y-3">
              ...
              <div className="border border-slate-200 rounded divide-y divide-slate-100">
                ...
                {/* Actual — editable by operator */}
                <div className="px-3 py-2">
                  <p className="text-xs font-medium text-slate-700 mb-1 flex items-center gap-1">
                    <CheckCheck className="w-3 h-3 text-green-600" />
                    Faktisk gjennomført endring <span className="font-normal text-slate-400">(skriv inn etter implementering)</span>
                  </p>
                  <textarea
                    rows={2}
                    value={actualChangeSummary}
                    onChange={(e) => setActualChangeSummary(e.target.value)}
                    placeholder="Beskriv hva som faktisk ble gjort — ikke kopier planlagt endring uten verifisering"
