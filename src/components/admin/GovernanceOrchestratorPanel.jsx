    ...
    <div className="space-y-4">
        ...
        <CardContent className="pt-0 space-y-3">
          ...
          {isInjected ? (
            <div className="flex items-start justify-between gap-2 bg-indigo-50 border border-indigo-200 rounded px-3 py-2">
              ...
              <button
                onClick={() => { onClearInjected?.(); setEnrichment({}); setShowOutputs(false); setConfirmedFiles(""); setActualChangeSummary(""); setVerificationNotes(""); setShowLogAssistant(false); setShowCreateConfirm(false); setCreateState(null); setCreateMode(null); setAnalysisConfirmed(false); }}
            ...
            <div>
              <select
                value={selectedId}
                onChange={(e) => { setSelectedId(e.target.value); setEnrichment({}); setShowOutputs(false); setConfirmedFiles(""); setActualChangeSummary(""); setVerificationNotes(""); setShowLogAssistant(false); setShowCreateConfirm(false); setCreateState(null); setCreateMode(null); setAnalysisConfirmed(false); }}
      ...
      {readyForVerification && (
        <div className="flex items-center justify-between gap-3 bg-green-50 border border-green-300 rounded-lg px-4 py-3">
          ...
          <button
            onClick={() => {
              setVerificationNotes(verificationTemplate);
        ...
        <Card>
          ...
          {showLogAssistant && (
            <CardContent className="pt-0 space-y-3">
              ...
              {/* Planned vs Actual change summary */}
              <div className="border border-slate-200 rounded divide-y divide-slate-100">
                {/* Planned — read-only, from audit */}
                <div className="px-3 py-2">
                  <p className="text-xs font-medium text-slate-500 mb-0.5 flex items-center gap-1">
                    <BookOpen className="w-3 h-3" />
                    Planlagt endring <span className="font-normal text-slate-400">(fra audit.requiredChange — skrivebeskyttet)</span>
                  </p>
                  <p className="text-xs text-slate-600 italic">
                    {audit.requiredChange ?? <span className="text-amber-600">Ikke spesifisert i audit.</span>}
                  </p>
                </div>
                {/* Actual — implementation notes, editable by operator */}
                <div className="px-3 py-2">
                  <p className="text-xs font-medium text-slate-700 mb-0.5 flex items-center gap-1">
                    <CheckCheck className="w-3 h-3 text-green-600" />
                    Faktisk implementert endring
                    <span className="font-normal text-slate-400 ml-1">(beskriv hva som ble endret i koden)</span>
                  </p>
                  <p className="text-xs text-slate-400 mb-1">Fyll inn etter implementering — beskriver hvilke endringer som faktisk ble gjort.</p>
                  <textarea
                    rows={2}
                    value={actualChangeSummary}
                    onChange={(e) => setActualChangeSummary(e.target.value)}
                    placeholder="Beskriv hva som faktisk ble endret — ikke kopier planlagt endring uten verifisering"
                  ...
                  />
                  {actualChangeSummary.trim()
                    ? <p className="text-xs text-green-600 mt-0.5 flex items-center gap-1"><CheckCheck className="w-3 h-3" />Bekreftet — vil bli brukt som diffSummary i utkastet.</p>
                    : <p className="text-xs text-amber-600 mt-0.5 flex items-center gap-1"><AlertTriangle className="w-3 h-3" />Ikke fylt inn — utkastet vil markere dette feltet som ubekreftet.</p>
                  }
                </div>
              </div>

              {/* Verification notes — only shown after PR merge */}
              {readyForVerification && (
                <div className="border border-green-200 rounded bg-green-50">
                  <div className="px-3 py-2 border-b border-green-200">
                    <p className="text-xs font-semibold text-green-800 flex items-center gap-1">
                      ...
                      <span className="font-normal text-green-700 ml-1">(post-merge review — ikke implementeringsnotes)</span>
                    </p>
                    <p className="text-xs text-green-700 mt-0.5">Bekreft at implementeringen fungerer som forventet etter at PR er merget. Dette er ikke en beskrivelse av hva som ble endret, men av hva som ble bekreftet.</p>
                  </div>
                  <div className="px-3 py-2">
                    <textarea
                      rows={4}
                      ...
                      placeholder="Beskriv hva som ble bekreftet etter merge — f.eks. «Verifisert fungerende», testresultater, observert oppførsel"
                      className="w-full text-xs border border-green-200 rounded px-2 py-1.5 bg-white focus:outline-none focus:border-green-400 resize-none"
                    />
                    {verificationNotes.trim()
                      ? <p className="text-xs text-green-700 mt-0.5 flex items-center gap-1"><CheckCheck className="w-3 h-3" />Verifiseringsnotat registrert.</p>
                      : <p className="text-xs text-green-600 mt-0.5 flex items-center gap-1"><ShieldCheck className="w-3 h-3" />Bruk «Prefill execution log» over for å forhåndsutfylle, eller skriv inn manuelt.</p>
                    }
                  </div>
                </div>
              )}
